...
public class MainPreferences {
    ...
    private static final int CORE_RANDOM_PORT = -1;
    private static final String DEFAULT = "/.default";
    private static final String FACTORY = "/factory";

    public static MainPreferences sInstance;
    ...
    public static synchronized MainPreferences instance() {
        if (sInstance == null) {
            sInstance = new MainPreferences();
        }
        return sInstance;
    }
    ...
    private void copyAssetsFromPackage() throws IOException {
        copyIfNotExist(getDefaultConfig());
        copyFromPackage(R.raw.rc_factory, new File(getFactoryConfig()).getName());
    }
    ...
    private void copyFromPackage(int resourceId, String target) throws IOException {
        ...
        InputStream lInputStream = mContext.getResources().openRawResource(resourceId);
        int readByte;
    ...
    public String getDefaultConfig() {
        return mBasePath + DEFAULT;
    }

    public String getFactoryConfig() {
        return mBasePath + FACTORY;
    }

    private String getString(int key) {
        if (mContext == null && MainCallService.isReady()) {
            mContext = MainCallService.getInstance().getApplicationContext();
        }
        return mContext.getString(key);
    }

    private Core getLc() {
        if (!MainCallService.isReady()) return null;

        return MainCallService.getCore();
    }
    ...
    public Config getConfig() {
        ...
        if (!MainCallService.isReady()) {
            File file = new File(mBasePath + "/.rc");
            if (file.exists()) {
                return Factory.instance().createConfig(file.getAbsolutePath());
            } else if (mContext != null) {
                InputStream inputStream =
                        mContext.getResources().openRawResource(R.raw.rc_default);
            ...
            return Factory.instance().createConfig(getDefaultConfig());
        }
        return null;
    }
    ...
    public void setAccountEnabled(int n, boolean enabled) {
        ...
        ProxyConfig prxCfg = getProxyConfig(n);
        if (prxCfg == null) {
            NurseCallUtils.printShort(mContext, "Error");
            return;
        }
        prxCfg.edit();
...
//    }
    ...
    public void setSipPort(int port) {
        ...
        transports.setTlsPort(CORE_RANDOM_PORT);
...
//        Log.d(TAG, "getDefaultDisplayName");
        if (getLc() == null) return null;
        return getLc().createPrimaryContactParsed().getDisplayName();
    }

    public String getDeviceName() {
        String defaultValue = Build.MANUFACTURER + " " + Build.MODEL;
    ...
    public void setTunnelHost(String host) {
        ...
        if (config != null) {
            config.setHost(host);
//            MainCallService.getInstance().initTunnelFromConf();
        }
    }
...
//        LinphoneManager.getInstance().initTunnelFromConf();
    ...
    public String getActivityToLaunchOnIncomingReceived() {
        if (getConfig() == null) return "org.linphone.activity_call.CallIncomingActivity";
        return getConfig()
                .getString(
                        "app", "incoming_call_activity", "org.linphone.activity_call.CallIncomingActivity");
    }
